    ...
    */
    ...
    calculateTooltipPosition() {
        if (this.rootElement) {
            ...
            let newRelatedY = this.task.index * this.rowHeight - this.scrollY + this.headerHeight;
            let newRelatedX: number;
            if (this.rtl) {
                newRelatedX = this.task.x1 - this.arrowIndent * 1.5 - tooltipWidth - this.scrollX;
                if (newRelatedX < 0) {
                    newRelatedX = this.task.x2 + this.arrowIndent * 1.5 - this.scrollX;
                }
                ...
                newRelatedX = this.task.x2 + this.arrowIndent * 1.5 + this.taskListWidth - this.scrollX;
                const tooltipLeftmostPoint = tooltipWidth + newRelatedX;
                const fullChartWidth = this.taskListWidth + this.svgContainerWidth;
                if (tooltipLeftmostPoint > fullChartWidth) {
                    newRelatedX = this.task.x1 + this.taskListWidth - this.arrowIndent * 1.5 - this.scrollX - tooltipWidth;
                }
                if (newRelatedX < this.taskListWidth) {
                    newRelatedX = this.svgContainerWidth + this.taskListWidth - tooltipWidth;
                    newRelatedY += this.rowHeight;
                }
            }
    ...
    render() {
        return (
            <div
                class={this.relatedX ? 'tooltipDetailsContainer' : 'tooltipDetailsContainerHidden'}
                style={{ left: `${this.relatedX}px`, top: `${this.relatedY}px` }}
            >
                <this.TooltipContent task={this.task} fontSize={this.fontSize} fontFamily={this.fontFamily} />
            </div>
        );
    }
}
