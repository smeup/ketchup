...
| `emitSubmitEventOnEnter` | `emit-submit-event-on-enter` | If text field has autocomplete associated and the list is opened, enter must not execute submit it serves just to set the selected item value of the list in the text field. | `boolean` | `true`      |
