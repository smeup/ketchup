...
export class KupCheckboxMenu {
    ...
    emitCheckboxSelectionUpdate() {
        ...
        this.kupCheckboxMenuSelected.emit(
            this.items.filter((item) => this.itemIsSelected(item))
        );
    }

    itemIsSelected(
        item: KupCheckboxMenuItem,
        fieldToCheck: string = this.valueField
    ): boolean {
        ...
        if (e.detail.checked) {
            ...
            if (this.itemIsSelected(item)) {
                ...
                this.selectedItems.splice(
                    this.selectedItems.indexOf(item[this.valueField]),
                    1
                );
                this.emitCheckboxSelectionUpdate();
            }
    ...
    toggleAllCheckboxesSelection(e: CustomEvent) {
        if (e.detail.checked) {
            ...
            ); // TODO if there are perfomance issues, then convert to standard for cycle
        } else {
            // Deselect all items
            this.selectedItems = [];
        }
    ...
    updateSelectedItemsArray(newValue: string, oldValue: string) {
        if (this.items && this.items.length) {
            ...
            for (let i = 0; i < this.items.length; i++) {
                if (
                    this.selectedItems.indexOf(this.items[i][oldValue]) >= 0 &&
    ...
    renderCheckboxList() {
        const itemsToRender =
            this.isFilterable && this.currentFilter.length
                ? basicListFilter(
                      this.items,
                      this.currentFilter,
                      this.displayedField
            ...
            <div class="checkbox-menu">
                <span
                    class={
                        ...
                        (this.disabled ? ' checkbox-menu__label--disabled' : '')
                    }
