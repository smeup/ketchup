...
const columnData = {
    ...
    visibleColumns() {
        return this.columns.reduce(
            (total, column) =>
                !column.hasOwnProperty(column.visible) || column.visible
                    ? total + 1
                    : total,
            0
        );
...
describe('data-table with sort column enabled', () => {
    beforeEach(async () => {
        ...
        await page.setContent(
            '<kup-data-table enable-sortable-columns></kup-data-table>'
        );
        ...
        draggableItems = await page.findAll(
            'kup-data-table >>> thead [draggable]'
        );
    });

    it.skip('can drag columns', async () => {
