...
describe.each([['checkbox', 'kup-checkbox'],['radio', 'kup-radio-element']])('data-table with a %s column', (compType, compElement) => {

  const checkObjectTypeFunction = compType === 'checkbox' ? isCheckbox : isRadio;
  ...
  const componentColumnName = columns.reduce((result, col) => col.title === 'Si/No' ? col.name: result, '');
  const componentColumnIndex = columns.reduce((result, col, index) => col.name === componentColumnName ? index : result, -1);
  ...
  beforeEach(async () => {
    ...
    tableCheckboxCells = await page.findAll(cellsSelector + `:nth-of-type(${componentColumnIndex + 1})`);
    return;
  });

  it(`renders ${compType} element only when specified by the cell`, async() => {
    await Promise.all(
      rows.map(async ({cells}, index) => {
        let cellCheckbox = await tableCheckboxCells[index].find(compElement);
        expect(!!cellCheckbox).toBe(checkObjectTypeFunction(cells[componentColumnName].obj));
      })
    );
  });
