...
describe('kup-data-table thead column context menu', () => {
    beforeEach(async () => {
        ...
        await page.setContent(
            '<span id="external-span">external cell content</span><kup-data-table></kup-data-table><div id="external-div">external div content</div>'
        );
        ...
        const coordinates = await page.evaluate(() => {
            let clientRect = document
                ...
                .shadowRoot.querySelector('thead > tr > th')
        ...
        menuElements = await page.findAll(
            'kup-data-table >>> thead .column-menu'
        );
    });

    it.skip('is opened by left click on th cells', async () => {
    ...
    describe('once opened,', () => {
        ...
        it.skip('is not automatically closed by a left click from within the open menu', async () => {
            // Checks that the first menu is open
            const openMenuBefore = await page.find(
                `kup-data-table >>> ${firstMenuSelector}`
            );
            ...
            const openMenuAfter = await page.find(
                `kup-data-table >>> ${firstMenuSelector}`
            );
        ...
        describe('is closed by a left click', () => {
            it.skip.each([
                ...
                'anywhere from within the table but not from within the menu itself (selector: %s)',
                async (currentSelector) => {
                    // Checks that the first menu is open
                    const openMenu = await page.find(
                        'kup-data-table >>> thead .column-menu'
                    );
                    ...
                    await page.evaluate((currentClickSelector) => {
                        (document
                            .querySelector('kup-data-table')
                            .shadowRoot.querySelector(
                                currentClickSelector
                            ) as HTMLTableCellElement).click();
                        return true;
                    }, currentSelector);
            ...
            it.skip.each([
                ...
                'from anywhere outside the kup-data-table (%s)',
                async (description, selector) => {
                    // Checks that the first menu is open
                    const openMenuBefore = await page.find(
                        `kup-data-table >>> ${firstMenuSelector}`
                    );
