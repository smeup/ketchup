<template>
    <div>
        ...
        <code>{{ JSON.stringify(hiddenColumnsData.columns) }}
            <br>
            <span ref="hiddenCols"/>
        </code>
        ...
        <code>{{ JSON.stringify(noAutomaticMutate.columns) }}
            <br>
        ...
        <code>{{ JSON.stringify(noAutomaticNewObject.columns) }}
            <br>
    ...
    methods: {
        ...
        handleKupDataTableNoAutomatic(reference, tableRef, e) {
            ...
            if (tableRef === 'noAutomaticMutate') {
                ...
                this.$refs[tableRef].defaultSortingFunction(this[tableRef].columns, e.detail.receivingColumnIndex, e.detail.sortedColumnIndex);
            } else {
                // Sort with creation of a new element
                this.$refs[tableRef].defaultSortingFunction(this[tableRef].columns, e.detail.receivingColumnIndex, e.detail.sortedColumnIndex, true)
                    .then((data) => {
                        this[tableRef] = {
                            ...this[tableRef],
                            columns: data
